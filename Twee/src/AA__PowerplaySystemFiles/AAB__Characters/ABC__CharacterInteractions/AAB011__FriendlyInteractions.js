var genericConversationTopics = {
    "The Weather": undefined,
    "The News": undefined,
    "Celebrity Gossip": undefined,
    "Politics": undefined,
    "TV": undefined,
    ...
    "Sports": undefined,
    "Food": undefined,
    "Refined Gastronomy (wines and cheeses)": undefined,
    "Shopping": undefined,
    "Fashion": undefined,
    "Traveling": undefined,
    "Electronics (phones, computers, etc.)": undefined,
    "Cars": undefined,
    "Pets": undefined,
    "Money": undefined,
    "People": undefined,
    "Local Rumors": undefined,
    "Partying": undefined,
    "Massages": undefined,
    "Drugs": undefined,
    "Health": undefined,
...
function getAdditionalRandomConversationTopics(maxTopicsToReturn, knownPreferredTopics, knownDislikedTopics, keyTopics){
    let result = new Array()
    let remainingTopicsToReturn = maxTopicsToReturn - (knownPreferredTopics.length + knownDislikedTopics.length + keyTopics.length)
    if (remainingTopicsToReturn <= 0){
        return result
    }
    let topicsKeys = genericConversationTopics.keys()
    const MAX_LOOPS = 100
    for (let loopCount = 0; (loopCount < MAX_LOOPS) && (remainingTopicsToReturn > result.length); loopCount++){
        let random = Math.floor(Math.random() * topicsKeys.length)
        let randomTopic = topicsKeys[random]
        if ((knownPreferredTopics.keys().find(randomTopic) == undefined)
            && (knownDislikedTopics.keys().find(randomTopic) == undefined)
            && (keyTopics.keys().find(randomTopic) == undefined)
            && (result.find(randomTopic) == undefined))
        {
...
class FriendlyInteraction extends window.CharacterInteraction {
    constructor(characters){
        super(characters)
        this.orderedBroachedTopics = new Array()
    }
    addBroachedTopic(topic){
        this.orderedBroachedTopics.push(topic)
    }
    checkBroachedTopics(topic){
        if (this.orderedBroachedTopics.find(topic) !== undefined){
            return true
        }
        return false
    }
    getLatestBroachedTopic(){
        if (this.orderedBroachedTopics.length <= 0){
            return undefined
        }
        return this.orderedBroachedTopics[this.orderedBroachedTopics.length -1]
    }
}
